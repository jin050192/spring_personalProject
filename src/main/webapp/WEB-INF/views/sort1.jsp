...
<div id="content">
	...
	<div class="list">		
		<ul>
			...
			<div class='textarea'>
			<p class='title'><a href="productInfo?num=${i.num}" style="font-size:14px;color:#555555;">${i.getName()}</a></p>
			<hr>
			...
			<th align="center">
				...
				<c:if test="${cnt > 0}">
					...
					<c:if test="${startPage > pageBlock}">
						<a href="sort1"> [◀◀] </a>
						<a href="sort1?pageNum=${startPage - pageBlock}"> [◀] </a>
					</c:if>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						...
						<c:if test="${i != currentPage}">
						<a href="sort1?pageNum=${i}">[${i}]</a>
						</c:if>
					</c:forEach>
					<!-- 다음블록[▶] / 마지막[▶▶] -->
					<c:if test="${pageCount > endPage}">
						<a href="sort1?pageNum=${startPage + pageBlock}"> [▶] </a>
						<a href="sort1?pageNum=${pageCount}"> [▶▶] </a>
					</c:if>
				</c:if>
			</th>
		</tr>
