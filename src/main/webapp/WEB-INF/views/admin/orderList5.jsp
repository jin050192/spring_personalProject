...
<div id="content">
    ...
    <div class="orderListArea">	
    	<form action="${pathA}/admin/confirmRefund">    	
		...
		<input type="submit" class="button" value="환불승인">
		<input type="submit" class="button" value="환불거부" formaction="${pathA}/admin/rejectRefund">
		</form>
	</div>
			...
			<th align="center">
				...
				<c:if test="${cnt > 0}">
					...
					<c:if test="${startPage > pageBlock}">
						<a href="${pathA}/admin/orderList5"> [◀◀] </a>
						<a href="${pathA}/admin/orderList5?pageNum=${startPage - pageBlock}"> [◀] </a>
					</c:if>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						...
						<c:if test="${i != currentPage}">
						<a href="${pathA}/admin/orderList5?pageNum=${i}">[${i}]</a>
						</c:if>
					</c:forEach>
					<!-- 다음블록[▶] / 마지막[▶▶] -->
					<c:if test="${pageCount > endPage}">
						<a href="${pathA}/admin/orderList5?pageNum=${startPage + pageBlock}"> [▶] </a>
						<a href="${pathA}/admin/orderList5?pageNum=${pageCount}"> [▶▶] </a>
					</c:if>
				</c:if>
			</th>
		</tr>
